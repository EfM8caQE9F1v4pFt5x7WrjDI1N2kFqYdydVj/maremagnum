...
app.whenReady().then(async () => {
  ...
  win.webContents.on('console-message', async (ev, ...args) => {
    ...
    fs.writeFileSync(path.join(OUT_DIR, 'mostri.webp'), Buffer.from(dataUrl.split(',')[1], 'base64'));
    fs.writeFileSync(path.join(OUT_DIR, 'mostri.json'), meta);
    const vecchio = path.join(OUT_DIR, 'mostri.png');
    if (fs.existsSync(vecchio)) fs.unlinkSync(vecchio);
    console.log('BAKE 📦 game/assets/mostri.webp + mostri.json');
    app.exit(0);
  });
  await win.loadFile(path.join(root, 'game', 'labmostri.html'));
});
